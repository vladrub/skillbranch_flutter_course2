...
void main() {
  ...
  group("l02h02", () {
    testWidgets(
        'Buttons with BodyParts have right background in unselected state',
        (WidgetTester tester) async {
      ...
      final coloredBoxInButtonFinder = (text) =>
          findTypeByTextOnlyInParentType(ColoredBox, text, BodyPartButton);
      ...
      final Set<Color> allColors =
          widgetsWithButtons.map((e) => e.color).toSet();
      expect(allColors, isNotEmpty);
      expect(allColors.length, 1);
      expect(allColors.single,
          isOneOrAnother(correctBackgroundColor, correctBackgroundColor2));
    });
  });
  ...
  group("l02h03", () {
    testWidgets(
      ...
      await tester.pumpWidget(MyApp());
      final GestureDetector goButton =
          tester.widget(find.widgetWithText(GestureDetector, "GO"));
      expect(goButton, isNotNull);
      final List<GestureDetector> widgetsWithBodyParts = tester
          .widgetList<GestureDetector>(
              find.widgetWithText(GestureDetector, "HEAD"))
          .toList();
      expect(widgetsWithBodyParts.length, 2);

      const Color selectedButtonColor = Color.fromRGBO(28, 121, 206, 1);

      final coloredBoxInGestureDetectorFinder = (text) =>
          findTypeByTextOnlyInParentType(ColoredBox, text, GestureDetector);

      final Color unselectedButtonColor = tester
          .widget<ColoredBox>(coloredBoxInGestureDetectorFinder("HEAD").first)
          .color;
      ...
      expect(
        tester
            .widget<ColoredBox>(coloredBoxInGestureDetectorFinder("HEAD").last)
            .color,
  ...
  group('l01h04', () {
    testWidgets(
      ...
      final List<GestureDetector> widgetsWithBodyParts = tester
          .widgetList<GestureDetector>(gestureDetectorFinder("HEAD"))
          .toList();
      expect(widgetsWithBodyParts.length, 2);
      ...
      final coloredBoxFinder =
          (text) => findTypeByTextOnlyInParentType(ColoredBox, text, Row);
  ...
  group('l01h05', () {
    testWidgets('There are 5 ones below You and Enemy',
        (WidgetTester tester) async {
      await tester.pumpWidget(MyApp());

      final List<Text> widgetsWithLegs =
          tester.widgetList<Text>(find.text("1")).toList();
